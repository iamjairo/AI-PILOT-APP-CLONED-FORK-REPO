...
import type { ArchivedChat, ChatExportOptions, ChatMessage } from './types';
  ...
  @media print{
    *{-webkit-print-color-adjust:exact !important;print-color-adjust:exact !important}
    ...
    .chat-content pre{white-space:pre-wrap !important;overflow-wrap:anywhere !important;
      ...
      /* Keep a code box whole on one page when it fits; a block taller than a
         page still breaks, but the dark One-Dark box/border stays continuous. */
...
/** Strip a captured filename to a safe basename (no path, no odd chars). */
function safeScriptName(name: string): string {
  const base = name.split(/[\\/]/).pop() || name;
  return base.replace(/[^\w.\-]+/g, '_').replace(/^_+|_+$/g, '').slice(0, 120);
}

/**
 * Collect code blocks for the scripts/ folder.
 *
 * SECURITY / CORRECTNESS: only ASSISTANT (model) messages contribute files. A
 * user's own message body is already rendered in the doc and must NEVER be
 * written out as a separate file. Prefer the real filename captured from the
 * code-block header (`block.name`); fall back to `message-<n>-block-<m>.<ext>`.
...
function collectCodeBlocks(chat: ArchivedChat): { name: string; code: string }[] {
  ...
  chat.messages.forEach((msg, mi) => {
    if (msg.role !== 'assistant') return; // never emit user-authored content
    (msg.codeBlocks ?? []).forEach((block, bi) => {
      let name = block.name ? safeScriptName(block.name) : '';
      if (!name) {
        const ext = extForLang(block.lang);
        name = `message-${mi + 1}-block-${bi + 1}.${ext}`;
      }
      // Deduplicate: foo.sh, foo-2.sh, foo-3.sh …
      ...
      files.push({ name, code: block.code });
    });
  });
  return files;
}
...
async function addChatToZip(
  ...
  // attachments/ — decode data: URLs / fetch remaining URLs into binary files.
  // Only ASSISTANT messages carry generated-file cards; a user's own uploads
  // are never written out. Every card is recorded in a manifest (captured or
  // missed) so gated/expired downloads are still accounted for.
  if (opts.downloadAttachments) {
    ...
    if (attachmentsDir) {
      ...
      let counter = 0;
      for (const msg of chat.messages) {
        if (msg.role !== 'assistant') continue; // never emit user uploads
        for (const att of msg.attachments ?? []) {
          counter += 1;
          let safeName = att.name?.replace(/[^\w.\-]+/g, '_') || `attachment-${counter}`;
          while (usedNames.has(safeName.toLowerCase())) safeName = `${counter}-${safeName}`;
          usedNames.add(safeName.toLowerCase());
          try {
            const res = await fetch(att.url);
            const buf = await res.arrayBuffer();
            attachmentsDir.file(safeName, buf);
            manifest.push(`[captured] ${safeName}`);
          } catch {
            // Bytes unreachable (gated / expired / CORS). Record, don't crash.
            manifest.push(`[missed]   ${att.name || safeName}  ${att.url || '(no url)'}`);
            // eslint-disable-next-line no-console
            console.warn('[AI Exporter] Could not fetch attachment:', att.name, att.url);
          }
        }
      }
      if (manifest.length > 0) {
        attachmentsDir.file(
          '_manifest.txt',
          `Generated files referenced in this conversation\n` +
            `(captured = bytes saved here; missed = only the filename is known)\n\n` +
            manifest.join('\n') +
            '\n',
        );
      }
